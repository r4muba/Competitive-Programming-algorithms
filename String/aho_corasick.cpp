...
string a[MAXN];
int lnk[MAXN],sz=1;
long long que[MAXN],endlink[MAXN]; 
vector<long long> leaf[MAXN],ans[MAXN];
void add_str(string s, int id) {
	...
	leaf[v].push_back(id); //Node in automata where a word ends
}
void push_links() {
	queue<int> q({0});
	while(!q.empty()) {
...
void walk(string s){ //KMP with multiple target patterns
	...
	for(int i = 0; i < (int)s.size(); i++){
		...
		for(int u=v;u;u=endlink[u]) for(int x: leaf[u]){
			ans[x].push_back(i); //pushing the index of the main string where a pattern ends
		}  
	}
}
void doit(){
