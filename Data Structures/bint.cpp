...
public:
    ...
    Bint(std::string& num) {
        int n{std::ssize(num)};
        for (int i{n}; i > 0; i -= 9) {
            d.push_back(std::stoi((i >= 9) ? num.substr(i - 9, 9) : num.substr(0, i)));
        }
    }
    Bint operator+(Bint& o) {
        Bint bint{}; auto& ans{bint.d}; int carry{};
    ...
    Bint operator*(int b) {
        ...
        while (c) ans.push_back(c % base); c /= base;
        bint.trim();
        return bint;
    }
    bool operator==(Bint& o) {
        if (size() != o.size()) return false;
        for (int i{}; i < size(); ++i) if (d[i] != o[i]) return false;
        return true;
    }
    bool operator<(Bint& o) {
        if (size() != o.size()) return size() < o.size();
        for (int i{size() - 1}; i >= 0; --i) if (d[i] != o[i]) return d[i] < o[i];
        return false;
    }
    std::string toString() {
        if (d.empty()) return "0";
